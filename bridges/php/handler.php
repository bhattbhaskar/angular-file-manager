...
if (Request::getApiParam('action') === 'list') {
    ...
    $data = [];
    $data["list"] = $list;
    $data["directory_count"] = $directory_count;
    $data["files_count"] = $file_count;
...
if (Request::getApiParam('action') === 'changePermissions') {
    ...
    $perms 		= Request::getApiParam('perms');

    //example
    foreach($items as $item) {
    	$result = $oFtp->changePermission($items, $permsCode, $perms) ;
    }
   // 
...
if (Request::getApiParam('action') === 'extract') {
    ...
    $zip                = Request::getApiParam('item');
    $destination        = Request::getApiParam('destination');
    $folder             = Request::getApiParam('folderName');
    ...
    $finalDest = preg_match('/\.(tar|zip)$/', $finalDest) ? $finalDest : $finalDest . '.zip';
    $result = $oFtp->extract($zip,$destination,$folder);
    
    $oResponse->setData(true);
    $oResponse->flushJson();
    
    if (! $result) {
        throw new Exception("Unknown error extracting this file");
    }
