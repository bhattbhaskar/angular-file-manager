...
class Ftp
{
     ...
     * @return  bool
     */
    public function extract($zip,$destination,$folder)
    {
        if (! $this->_isConnected()) {
            return false;
        }
        ...
        shell_exec("unzip /home/guruqcqw/public_html/guruquest.net/ruby".$zip." -d /home/guruqcqw/public_html/guruquest.net/ruby".$destination); 
 ...
 /**
     ...
     * @return  bool
     */
    public function changePermission($items, $permsCode, $perm) {
        if (! $this->_isConnected()) {
            return false;
        }

        foreach($items as $item) {
        	$this->chmod($item ,octdec($permsCode));
        	//chmod("/home/guruqcqw/public_html/guruquest.net/ruby".$item,octdec($permsCode));
        }
   	return true;     
   }
    // --------------------------------------------------------------------
    ...
    public function listFilesRaw($path = '.', $onlyDirectories = false, array $excludeIf = array('.', '..'), $includeExtensionFileOnlyIf = array())
    {
        ...
        $result = false;
	ftp_chdir($this->_conn, $path);

        if (is_array($children = @ftp_rawlist($this->_conn,'-a .'))) {
            $items = array();
            $folder = array();
